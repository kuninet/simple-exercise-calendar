...
function handleDatabaseError(err, res, operation = 'データベース操作') {
  ...
  if (err.code === 'SQLITE_BUSY') {
    return res.status(503).json({
      success: false,
      error:
        'データベースが一時的に利用できません。しばらく待ってから再試行してください。',
      code: 'DATABASE_BUSY'
    })
...
// API: 疎通確認（クライアントのオンライン・サーバー稼働チェック用）
app.get('/api/ping', (req, res) => {
  res.json({ status: 'ok', timestamp: getCurrentJSTTimestamp() })
})

// API: データ同期（ローカルファーストデータのサーバー反映）
      ...
      try {
        ...
        db.serialize(() => {
          db.run('BEGIN TRANSACTION', (beginErr) => {
            if (beginErr) {
              handleDatabaseError(beginErr, res, '同期トランザクション開始')
              return resolve()
            }
            ...
            if (Array.isArray(exercises) && exercises.length > 0) {
              ...
              INSERT INTO exercises (id, name, category, unit, icon, user_id, is_active)
              VALUES (?, ?, ?, ?, ?, ?, ?)
              ON CONFLICT(id) DO UPDATE SET
                name = excluded.name,
            ...
            // 3. 削除対象レコードの処理
            if (
              Array.isArray(deletedRecordIds) &&
              deletedRecordIds.length > 0
            ) {
              const placeholders = deletedRecordIds.map(() => '?').join(',')
              db.run(
                `DELETE FROM exercise_records WHERE id IN (${placeholders})`,
                deletedRecordIds
              )
            }
            ...
            if (Array.isArray(records) && records.length > 0) {
              ...
              INSERT INTO exercise_records (user_id, exercise_id, record_date, is_quick_record, notes, created_at)
              VALUES (?, ?, ?, ?, ?, ?)
              ON CONFLICT(user_id, exercise_id, record_date) DO UPDATE SET
            ...
            `)
              for (const r of records) {
                recStmt.run(
                  ...
                  r.exercise_id,
                  r.record_date,
                  r.is_quick_record ? 1 : 0,
                  r.notes || '',
                  r.created_at || getCurrentJSTTimestamp()
                )
              }
              recStmt.finalize()
            }
            ...
            db.run('COMMIT', (commitErr) => {
              if (commitErr) {
                db.run('ROLLBACK', () => {
                  handleDatabaseError(commitErr, res, 'データ同期コミット')
                  resolve()
                })
                return
              }

              const serverTimestamp = getCurrentJSTTimestamp()
              console.log(`✅ データ同期が完了しました (${serverTimestamp})`)
              res.json({
                success: true,
                status: 'ok',
                ...
                synced: {
                  ...
                  exercisesCount: Array.isArray(exercises)
                    ? exercises.length
                    : 0,
        ...
        console.error('同期処理例外:', error)
        res.status(500).json({
          success: false,
          error: error.message || 'データ同期中にエラーが発生しました'
        })
        resolve()
      }
    })

  syncMutex = syncMutex.then(executeSync, executeSync)
})
...
app.get('/api/backup', (req, res) => {
  try {
    ...
    const backupPath = path.join(
      backupDir,
      `exercise-app-backup-${timestamp}.db`
    )
...
app.get('/api/export', (req, res) => {
  try {
    ...
    db.all(exportQuery, [userId], (err, records) => {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="exercise-data-user${userId}-${new Date().toISOString().split('T')[0]}.json"`
      )
      res.json(exportData)
    })
  } catch (error) {
...
app.get('/api/records', (req, res) => {
  try {
    ...
    const params = []
    if (userId) {
      query += ' WHERE er.user_id = ?'
      params.push(userId)
    }
    query += ' ORDER BY er.record_date DESC'
    ...
    db.get(userQuery, [userId], (err, user) => {
      ...
      db.get(checkQuery, [userId, date], (err, row) => {
        ...
        db.run('BEGIN TRANSACTION', (err) => {
          ...
          db.run(
            insertQuery,
            [userId, defaultExerciseId, date, getCurrentJSTTimestamp()],
            function (err) {
              if (err) {
                db.run('ROLLBACK')
              ...
              db.run('COMMIT', (err) => {
                ...
                console.log(
                  `✅ 日付記録を追加しました: ユーザー${userId}, エクササイズ${defaultExerciseId}, 日付${date}`
                )
...
app.put('/api/user/name', (req, res) => {
  try {
    ...
    db.run(updateQuery, [displayName, userId], function (err) {
      ...
      console.log(
        `✅ ユーザー${userId}の名前を「${displayName}」に更新しました`
      )
      res.json({
        success: true,
  ...
  try {
    ...
    db.run(
      ...
      function (err) {
        ...
        console.log(
          `✅ 新しいユーザーを追加しました: ${displayName} (ID: ${this.lastID})`
        )
        res.json({
          success: true,
          message: 'ユーザーを追加しました',
...
app.put('/api/user/default-exercise', (req, res) => {
  try {
    ...
    db.run(updateQuery, [exerciseId, userId], function (err) {
      ...
      console.log(
        `✅ ユーザー${userId}のデフォルトエクササイズを${exerciseId}に更新しました`
      )
      res.json({
        success: true,
...
app.post('/api/add-exercise', (req, res) => {
  try {
    ...
    db.get(checkQuery, [userId, exerciseId, date], (err, row) => {
      ...
      db.run(
        insertQuery,
        [userId, exerciseId, date, getCurrentJSTTimestamp()],
        function (err) {
          if (err) {
          ...
          console.log(
            `✅ エクササイズを追加しました: ユーザー${userId}, エクササイズ${exerciseId}, 日付${date}`
          )
          res.json({
            success: true,
  ...
  try {
    ...
    if (!userId || !exerciseId || !date) {
      return res.status(400).json({
        success: false,
        error:
          '必要なパラメータが不足しています（ユーザーID、エクササイズID、日付）',
          ...
          db.run(
            ...
            function (err) {
              ...
              db.run('COMMIT', (err) => {
                ...
                console.log(
                  `✅ 新しい記録を追加しました: ユーザー${userId}, エクササイズ${exerciseId}, 日付${date}`
                )
